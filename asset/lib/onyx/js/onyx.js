...
!(function ($) {
    ...
    Onyx = {
        ...
        /**
        * Change the upload URL dynamically
        * @param {string} newUrl - The new URL for file uploads
        */
        changeUploadUrl: function (newUrl) {
            var onyxDropzone = Dropzone.forElement('form.dropzone');
            if (onyxDropzone) {
                onyxDropzone.options.url = newUrl;
            }
        },
        ...
        userFilesDropzone: {
            ...
            initFileUploader: function (base, target) {
                ...
                var onyxDropzone = new Dropzone(target, {
                    url: ($(target).attr("action")) ? $(target).attr("action") : "/upload-attachment?projectId=0", // Check that our form has an action attr and if not, set one here
                ...
                onyxDropzone.on('removedfile', function (file) {
                    $.ajax({
                        type: 'post',
                        //url: ($(target).attr("action")) ? $(target).attr("action") : "/Admin/Post/UploadAttachments?postId=0", // Check that our form has an action attr and if not, set one here
                        data: {
    ...
    $(document).ready(function () {
        ...
        function updateOnyxAction(postId) {

            var url = "/upload-attachment?projectId=" + postId;

            $("#attachment-modal").attr('data-projectid', postId);

            $("#attachment-modal form.dropzone").attr('action', url);

            Onyx.changeUploadUrl(url);
        }

        $(".open-attachment").click(function () {
            var projectId = $(this).attr("data-projectid");
            var existingDropzone = Dropzone.forElement('form.dropzone');
            if (existingDropzone) {
                existingDropzone.destroy();
            }

            Onyx.init();
            updateOnyxAction(projectId);
        });
